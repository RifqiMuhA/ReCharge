...
const ScrollStack = ({
  ...
  const getElementOffset = useCallback(
    (element, index) => {
      if (useWindowScroll) {
        ...
        let translateY = 0;
        if (index !== undefined && lastTransformsRef.current.has(index)) {
          translateY = lastTransformsRef.current.get(index).translateY || 0;
        }
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Global window scroll relies on the global Lenis already existing from SmoothScroll provider
