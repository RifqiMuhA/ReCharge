...
export default function ContactPage() {
            ...
            if (flightPathRef.current && birdContainerRef.current) {
                ...
                masterTl.addLabel("phase1")
                    .fromTo(letterRef.current,
                        ...
                        { y: "15vh", opacity: 1, rotationZ: 0, duration: 2, ease: "power2.out" }, "phase1") // falls closer to bird
                    .fromTo(textKirimPesanRef.current,
                        { x: -50, opacity: 0 },
                        { x: 0, opacity: 1, duration: 1.5, ease: "power2.out" }, "phase1+=0.5")
                    .fromTo(textKeSemuaRef.current,
                        { x: 50, opacity: 0 },
                        { x: 0, opacity: 1, duration: 1.5, ease: "power2.out" }, "phase1+=0.5");

                // Phase 2: Letter reaches bird -> Bird changes to holding letter. NO flying yet.
                masterTl.addLabel("phase2", "+=0.3")
                    ...
                    .to(birdNormalContainerRef.current, { opacity: 0, duration: 0.1 }, "phase2")
                    .to(birdLetterContainerRef.current, { opacity: 1, duration: 0.1 }, "phase2");

                // Phase 2.5: Cage hides, texts hide, bird starts flapping and liftoff.
                masterTl.addLabel("phase2_5", "+=1") // Wait a moment before flying
                    .to([birdCageRef.current, textKirimPesanRef.current, textKeSemuaRef.current],
                        { opacity: 0, duration: 1.5, ease: "power2.inOut" }, "phase2_5")
                    // Initiate "flying" by moving up slightly (omitting scale to prevent GSAP matrix conflicts)
                    .to(birdFlightRef.current, { y: "-5vh", duration: 2, ease: "power1.inOut" }, "phase2_5");

                // Phase 3: "Perasaanmu" and "Berharga" Awwwards style text animation
                masterTl.addLabel("phase3", "+=0.5")
                    .fromTo(textPerasaanmuRef.current,
                        { y: 50, opacity: 0, rotationX: -90 },
                        { y: 0, opacity: 1, rotationX: 0, transformOrigin: "bottom center", duration: 1.5, ease: "back.out(1.7)" }, "phase3")
                    .fromTo(textBerhargaRef.current,
                        { y: 50, opacity: 0, rotationX: -90 },
                        { y: 0, opacity: 1, rotationX: 0, transformOrigin: "bottom center", duration: 1.5, ease: "back.out(1.7)" }, "phase3+=0.3")
                    // Let them stay a bit, then fade out
                    .to([textPerasaanmuRef.current, textBerhargaRef.current],
                        { y: -50, opacity: 0, duration: 1.5, ease: "power2.in" }, "+=2"); // Stay longer

                // Phase 4: Bird lands on ranting.
                masterTl.addLabel("phase4", "+=0.5")
                    ...
                    .fromTo(birdRantingRef.current,
                        { opacity: 0, scaleY: 1.2, scaleX: -1.2 },
                        { opacity: 1, scaleY: 1.2, scaleX: -1.2, duration: 0.01 }, "phase4+=0.01");
                ...
                flapTl.to(birdFramesLetterRef.current[0], { opacity: 0, duration: 0 }, "+=0.15")
                    .to(birdFramesLetterRef.current[1], { opacity: 1, duration: 0 }, "<")
                    .to(birdFramesLetterRef.current[1], { opacity: 0, duration: 0 }, "+=0.15")
                    .to(birdFramesLetterRef.current[2], { opacity: 1, duration: 0 }, "<")
                    .to(birdFramesLetterRef.current[2], { opacity: 0, duration: 0 }, "+=0.15")
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!formData.name || !formData.email || !formData.location || !formData.intent) {
    ...
    return (
        <div className="w-full bg-[#F5F5ED] overflow-hidden relative cursor-auto box-border">
                ...
                <div className="max-w-[85rem] w-full mx-auto flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-16">
                    ...
                    <div className="w-full lg:w-8/12 relative mt-8 lg:mt-0 px-2 sm:px-4 lg:px-8" style={{ perspective: "1500px" }}>
                        ...
                        <div ref={mainPaperRef} className="bg-[#FAF9F6] shadow-[0_10px_30px_rgba(0,0,0,0.06)] rounded-sm border border-[#E5E5DF] relative transform-style-3d">
                            ...
                            <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-y-7 text-base sm:text-xl md:text-2xl lg:text-3xl font-geometric font-light leading-relaxed text-pine-teal/90 px-4 py-8 sm:px-8 sm:py-10 md:px-16 md:py-12 relative z-10 w-full box-border">
                                ...
                                <div className="flex flex-nowrap items-end gap-x-2 sm:gap-x-3 w-full">
                                    ...
                                    <div className={`relative inline-block group border-b ${hasSubmitted && !formData.name ? 'border-red-500' : 'border-pine-teal/30 focus-within:border-pine-teal'} transition-colors flex-grow min-w-[40px] md:max-w-[300px]`}>
                                        <input
                                            type="text"
                                            name="name"
                                            value={formData.name}
                                        ...
                                        {hasSubmitted && !formData.name && <span className="absolute -bottom-5 left-0 right-0 text-center text-[9px] sm:text-[10px] font-geometric font-bold tracking-widest text-red-500 uppercase whitespace-nowrap translate-y-2">Wajib Diisi</span>}
                                    </div>
                                    <span className="pb-1 shrink-0 whitespace-nowrap">dari</span>
                                    <div className={`relative inline-block group border-b ${hasSubmitted && !formData.location ? 'border-red-500' : 'border-pine-teal/30 focus-within:border-pine-teal'} transition-colors flex-grow min-w-[40px] md:max-w-[250px]`}>
                                        <input
                                            type="text"
                                            name="location"
                                            ...
                                            className="w-full bg-transparent outline-none text-center text-pine-teal font-normal pb-0.5 placeholder-pine-teal/20 text-base md:text-lg tracking-tight relative z-10 cursor-none min-w-0"
                                        />
                                        {hasSubmitted && !formData.location && <span className="absolute -bottom-5 left-0 right-0 text-center text-[9px] sm:text-[10px] font-geometric font-bold tracking-widest text-red-500 uppercase whitespace-nowrap translate-y-2">Wajib Diisi</span>}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-nowrap items-end gap-x-2 sm:gap-x-4 w-full">
                                    ...
                                    <div className={`relative inline-block group border-b ${hasSubmitted && (!formData.email || (formData.email && !formData.email.includes('@'))) ? 'border-red-500' : 'border-pine-teal/30 focus-within:border-pine-teal'} transition-colors flex-grow min-w-[50px] md:max-w-[320px]`}>
                                        <input
                                            type="email"
                                            name="email"
                                            ...
                                            className="w-full bg-transparent outline-none text-center text-pine-teal font-normal pb-0.5 placeholder-pine-teal/20 text-base md:text-lg tracking-tight relative z-10 cursor-none min-w-0"
                                        />
                                        {hasSubmitted && !formData.email && <span className="absolute -bottom-5 left-0 right-0 text-center text-[9px] sm:text-[10px] font-geometric font-bold tracking-widest text-red-500 uppercase whitespace-nowrap translate-y-2">Wajib Diisi</span>}
                                ...
                                <div className="flex flex-nowrap items-end gap-x-2 sm:gap-x-3 w-full">
                                    ...
                                    <div className={`relative inline-block group border-b ${hasSubmitted && !formData.intent ? 'border-red-500' : 'border-pine-teal/30 focus-within:border-pine-teal'} transition-colors flex-grow min-w-[50px]`}>
                                        <input
                                            type="text"
                                            name="intent"
                                        ...
                                        />
                                        {hasSubmitted && !formData.intent && <span className="absolute -bottom-5 left-0 right-0 text-center text-[9px] sm:text-[10px] font-geometric font-bold tracking-widest text-red-500 uppercase whitespace-nowrap translate-y-2">Wajib Diisi</span>}
                                    </div>
                                </div>
            ...
            <div ref={flightPathRef} className="w-full relative bg-[#F5F5ED] overflow-hidden">

                {/* Center pinned container */}
                <div ref={birdContainerRef} className="w-full h-[180px] md:h-[220px] flex flex-col items-center justify-center relative z-40">
                    ...
                    <div className="absolute inset-0 flex items-center justify-between pointer-events-none z-50 overflow-hidden perspective-1000">
                        {/* Phase 1 Texts */}
                        <div ref={textKirimPesanRef} className="text-3xl sm:text-4xl md:text-6xl font-geometric text-pine-teal/80 absolute left-4 sm:left-8 md:left-24 top-[40%] md:top-1/2 -translate-y-1/2 opacity-0 whitespace-nowrap">
                            kirim pesan
                        </div>
                        <div ref={textKeSemuaRef} className="text-3xl sm:text-4xl md:text-6xl font-geometric text-pine-teal/80 absolute right-4 sm:right-8 md:right-24 top-[40%] md:top-1/2 -translate-y-1/2 opacity-0 whitespace-nowrap">
                            ke semua orang
                        </div>

                        {/* Phase 3 Texts (Awwwards Style) */}
                        <div ref={textPerasaanmuRef} className="text-4xl sm:text-5xl md:text-8xl font-geometric font-bold text-blush-pop absolute left-4 sm:left-8 md:left-24 top-[40%] md:top-1/2 -translate-y-1/2 opacity-0 whitespace-nowrap" style={{ transformStyle: 'preserve-3d' }}>
                            perasaanmu
                        </div>
                        <div ref={textBerhargaRef} className="text-4xl sm:text-5xl md:text-8xl font-geometric font-bold text-blush-pop absolute right-4 sm:right-8 md:right-24 top-[40%] md:top-1/2 -translate-y-1/2 opacity-0 whitespace-nowrap" style={{ transformStyle: 'preserve-3d' }}>
                            berharga
                        </div>
                    </div>

                    {/* Falling Letter */}
                    <div ref={letterRef} className="absolute z-40 w-10 h-6 bg-white border border-gray-200 shadow-md flex items-center justify-center rotate-12 opacity-0 top-[28%] -mt-10 mr-12">
                    ...
                    <div className="relative w-48 h-64 sm:w-56 sm:h-72 flex items-center justify-center">
                        <div ref={birdCageRef} className="absolute inset-0 z-10 translate-y-16">
                            <Image src={sarangBurungImg} alt="Sarang Burung" fill className="object-contain pointer-events-none" />
                        </div>

                        {/* The Bird superimposed on the cage */}
                        <div ref={birdFlightRef} className="absolute w-28 h-28 sm:w-36 sm:h-36 z-30 translate-y-0">

                            {/* Phase 0 & 1 Container: Normal Bird */}
                            <div ref={birdNormalContainerRef} className="absolute inset-0">
                                <Image
                                    ref={el => { birdFramesNormalRef.current[0] = el; }}
                                    src={bird1Img} alt="Bird normal 1" fill className="object-contain pointer-events-none opacity-100 -scale-x-100"
                                />
                                <Image
                                    ref={el => { birdFramesNormalRef.current[1] = el; }}
                                    src={bird2Img} alt="Bird normal 2" fill className="object-contain pointer-events-none opacity-0 -scale-x-100"
                                />
                                <Image
                                    ref={el => { birdFramesNormalRef.current[2] = el; }}
                                    src={bird3Img} alt="Bird normal 3" fill className="object-contain pointer-events-none opacity-0 -scale-x-100"
                                />
                            </div>

                            {/* Phase 2 & 3 Container: Bird with Letter */}
                            <div ref={birdLetterContainerRef} className="absolute inset-0 opacity-0">
                                <Image
                                    ref={el => { birdFramesLetterRef.current[0] = el; }}
                                    src={birdLetter1Img} alt="Bird letter 1" fill className="object-contain pointer-events-none opacity-100 -scale-x-100"
                                />
                                <Image
                                    ref={el => { birdFramesLetterRef.current[1] = el; }}
                                    src={birdLetter2Img} alt="Bird letter 2" fill className="object-contain pointer-events-none opacity-0 -scale-x-100"
                                />
                                <Image
                                    ref={el => { birdFramesLetterRef.current[2] = el; }}
                                    src={birdLetter3Img} alt="Bird letter 3" fill className="object-contain pointer-events-none opacity-0 -scale-x-100"
                                />
                            </div>
                        </div>
                        ...
                        <div className="absolute w-28 h-28 sm:w-36 sm:h-36 z-30 translate-y-0 pointer-events-none">
                            <Image
                                ref={birdRantingRef}
                                src={birdRantingImg} alt="Bird ranting" fill className="object-contain pointer-events-none opacity-0"
                            />
                        </div>
                    </div>
                </div>
            ...
            <div className="w-full relative flex flex-col items-center justify-center pb-32 overflow-hidden bg-[#F5F5ED]">
                <div className="max-w-7xl mx-auto text-center px-4 md:px-8 z-10 w-full relative">
                    ...
                    <p className="text-base md:text-xl text-pine-teal/70 max-w-3xl mx-auto mb-16 font-tt-commons-light">
                        Kamu tidak sendirian, selalu ada orang baik di sekitar mu bahkan di seluruh dunia yang saling peduli
                    </p>

                    {/* World Map Component */}
                    <div className="w-full max-w-5xl mx-auto opacity-90 relative z-20">
                        <WorldMap
                            lineColor="#8DDEDE" // pearl-aqua
                            dots={[
                                {
                                    start: { lat: -6.2088, lng: 106.8456, imageUrl: "https://images.unsplash.com/photo-1555899434-94d1368aa7af?q=80&w=400&h=400&fit=crop", label: "Jakarta" }, // Jakarta, Indonesia
                                    end: { lat: 35.6762, lng: 139.6503, imageUrl: "https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?q=80&w=400&h=400&fit=crop", label: "Tokyo" }, // Tokyo, Japan
                                },
                                {
                                    start: { lat: -6.2088, lng: 106.8456, imageUrl: "https://images.unsplash.com/photo-1555899434-94d1368aa7af?q=80&w=400&h=400&fit=crop", label: "Jakarta" }, // Jakarta
                                    end: { lat: 51.5074, lng: -0.1278, imageUrl: "https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?q=80&w=400&h=400&fit=crop", label: "London" }, // London, UK
                                },
                                {
                                    start: { lat: -6.2088, lng: 106.8456, imageUrl: "https://images.unsplash.com/photo-1555899434-94d1368aa7af?q=80&w=400&h=400&fit=crop", label: "Jakarta" }, // Jakarta
                                    end: { lat: -33.8688, lng: 151.2093, imageUrl: "https://images.unsplash.com/photo-1506973035872-a4ec16b8e8d9?q=80&w=400&h=400&fit=crop", label: "Sydney" }, // Sydney, Australia
                                },
                                {
                                    start: { lat: -6.2088, lng: 106.8456, imageUrl: "https://images.unsplash.com/photo-1555899434-94d1368aa7af?q=80&w=400&h=400&fit=crop", label: "Jakarta" }, // Jakarta
                                    end: { lat: 40.7128, lng: -74.0060, imageUrl: "https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?q=80&w=400&h=400&fit=crop", label: "New York" }, // New York, USA
                                },
                                {
                                    start: { lat: -6.2088, lng: 106.8456, imageUrl: "https://images.unsplash.com/photo-1555899434-94d1368aa7af?q=80&w=400&h=400&fit=crop", label: "Jakarta" }, // Jakarta
                                    end: { lat: 25.2048, lng: 55.2708, imageUrl: "https://images.unsplash.com/photo-1512453979798-5ea266f8880c?q=80&w=400&h=400&fit=crop", label: "Dubai" }, // Dubai, UAE
                                },
                            ]}
                        />
                    </div>
